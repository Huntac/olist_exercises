...
if __name__ == '__main__':
    ...
    try:
        ...
        reviews_df['label'] = pd.cut(reviews_df['review_score'], bins = [0,2,5], labels = [1,0]) 
        ...
        tfidf_class_pipe = Pipeline([
            ...
            ('bow', CountVectorizer(stop_words=STOPWORDS, ngram_range=(1,1))),
            ('tfidf', TfidfTransformer()),
            ('rf_clf', RandomForestClassifier(class_weight='balanced', random_state = RANDOM_STATE))
        ])

        params = {
            'bow__max_df': (0.15, 0.35, 0.5),
            #'rf_clf__n_estimators': (100, 300),
        ...
        grid = GridSearchCV(tfidf_class_pipe, params, scoring = scorer, verbose = 3, cv = 3)
        ...
        val_report = {'validation_metrics': classification_report(y_val, val_preds, output_dict = True),
                      'training_metrics': classification_report(y_train, train_preds, output_dict = True)}
        ...
        if hasattr(grid.best_estimator_.named_steps['rf_clf'], 'feature_importances_'):
            feat_imp = grid.best_estimator_.named_steps['rf_clf'].feature_importances_
            feat_names = grid.best_estimator_.named_steps['bow'].get_feature_names()
        val_report['feature_importances'] = dict(zip(feat_names,feat_imp))
        ...
        report_path = os.path.join('.','sentiment_analysis', 'reports', report_file)
